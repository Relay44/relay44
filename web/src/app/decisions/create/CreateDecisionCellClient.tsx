...
export default function CreateDecisionCellClient() {
  ...
  const [actions, setActions] = useState(() =>
    defaultActionsForDecisionType("timing"),
  );

  const starterNodes = useMemo(
    () => STARTER_NODE_COPY[decisionType],
    [decisionType],
  );
  const usesDefaultTimingActions = decisionType === "timing";

  useEffect(() => {
    setActions(defaultActionsForDecisionType(decisionType));
  }, [decisionType]);

  const handleActionChange = (index: number, next: string) => {
    setActions((current) =>
      current.map((value, valueIndex) => (valueIndex === index ? next : value)),
    );
  };
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!usesDefaultTimingActions && (nextActions?.length ?? 0) < 2) {
      addToast(
        "Choice, hedge, and allocation cells need at least two actions.",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      addToast(
        error instanceof Error
          ? error.message
          : "Failed to create decision cell.",
        "error",
      );
    }
  };
        ...
        <div className="mx-auto max-w-4xl space-y-6">
          <div className="max-w-2xl">
            ...
            <h1 className="mt-2 text-3xl font-semibold text-text-primary">
              Create a decision cell
            </h1>
            <p className="mt-3 text-sm text-text-secondary">
              Define the decision, choose the action set, and create starter
              nodes you can connect to live markets and external agents.
            </p>
          </div>
          ...
          >
              ...
              <div className="space-y-3">
                ...
                {usesDefaultTimingActions ? (
                  <div className="grid gap-3 sm:grid-cols-2">
                    {defaultActionsForDecisionType("timing").map((action) => (
                      <div
                        key={action}
                        className="border border-border bg-bg-secondary px-3 py-3 text-sm text-text-primary"
                      >
                        {action}
                      </div>
                    ))}
                  ...
                  <div className="grid gap-3">
                    <Input
                      ...
                      value={actions[0]}
                      onChange={(event) =>
                        handleActionChange(0, event.target.value)
                      }
                    ...
                    <Input
                      ...
                      value={actions[1]}
                      onChange={(event) =>
                        handleActionChange(1, event.target.value)
                      }
                    ...
                    <Input
                      ...
                      value={actions[2]}
                      onChange={(event) =>
                        handleActionChange(2, event.target.value)
                      }
            ...
            <Card className="space-y-4">
              ...
              <p className="text-sm text-text-secondary">
                After creation, attach live markets, set action effects,
                configure alerts, and connect external agents.
              </p>
            </Card>
          </form>
        </div>
