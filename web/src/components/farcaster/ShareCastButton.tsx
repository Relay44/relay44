...
export const ShareCastButton: FC<ShareCastButtonProps> = ({
  ...
  return (
    <button
      onClick={handleShare}
      className={`inline-flex items-center gap-1.5 rounded-md border border-border bg-surface px-3 py-1.5 text-xs font-medium text-muted hover:bg-surface-hover hover:text-foreground transition-colors ${className ?? ''}`}
    >
