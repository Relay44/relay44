...
import { formatPercentFromBps, scoreBarWidth } from "@/lib/decisionCells";
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between gap-3 text-sm">
        <div className="min-w-0">
          ...
          {typeof rank === "number" ? (
            <span className="ml-2 text-xs uppercase tracking-[0.14em] text-text-muted">
              #{rank + 1}
            </span>
          ) : null}
        </div>
      ...
      <div className="relative h-3 overflow-hidden border border-border bg-bg-secondary">
        ...
        <div
          ...
          style={{ width: `${scoreBarWidth(clamped) / 2}%` }}
        />
      </div>
    </div>
  );
