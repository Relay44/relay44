import type { UAL, PublishResult, DKGClientConfig } from '../types/index.js';
...
export interface DKGClientInterface {
  ...
  publish(content: object, options?: { epochs?: number; paranetUAL?: string }): Promise<string>;
...
export class DKGClient implements DKGClientInterface {
  ...
  private async ensureInitialized(): Promise<void> {
    ...
    try {
      ...
      const DKGConstructor = (DKG as any).default || DKG;

      this.client = new DKGConstructor({
  ...
  async update(ual: UAL, data: Record<string, unknown>): Promise<void> {
    ...
    return withRetry(
      async () => {
        ...
        await this.client.asset.update(ual, { public: data });
      },
      this.config.maxRetries!,
      this.config.retryDelayMs!,
      this.logger,
    ...
    return withRetry(
      async () => {
        ...
        const result = await this.client.asset.create(
          { public: content },
          {
            epochsNum: options?.epochs || 12,
            ...(options?.paranetUAL ? { paranetUAL: options.paranetUAL } : {}),
          },
        );

        const ual = result?.UAL;
        if (!ual) throw new Error('No UAL returned from DKG publish');
...
export class MockDKGClient implements DKGClientInterface {
  ...
  async get(ual: UAL): Promise<{ content: unknown; metadata?: Record<string, unknown> }> {
    ...
    if (!asset) return { content: {}, metadata: {} };
    return asset;
  }

  async update(ual: UAL, data: Record<string, unknown>): Promise<void> {
    const asset = this.assets.get(ual);
    if (asset) asset.metadata = { ...asset.metadata, ...data };
  }

  async publish(content: object): Promise<string> {
    const ual = `did:dkg:base/8453:relay44/${++this.counter}`;
...
export function createDKGClient(
  ...
  if (!endpoint) {
    if (logger) logger.warn('No DKG endpoint configured, using mock client');
  ...
  const rpc =
    ...
    process.env.CONTEXT_GRAPH_DKG_RPC_URL;

  const fullConfig: DKGClientConfig = {
    endpoint,
    port: config?.port || parseInt(process.env.DKG_PORT || '8900'),
    blockchain: config?.blockchain || {
      name: process.env.DKG_BLOCKCHAIN || DKG_NETWORKS.BASE_MAINNET,
